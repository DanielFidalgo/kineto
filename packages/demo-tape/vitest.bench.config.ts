...
// practice (~5-6s locally), but CI runners can be slow/noisy and this is
// a diagnostic, not a gate: 120s gives a loaded free-tier runner room to
// finish and actually log numbers, rather than timing out. The CI step
// also has `continue-on-error: true` (see ci.yml) as a second layer in
// case a runner is slow enough to blow even this budget.
